...
final class AppLaunch {
    ...
    func start() async {
        ...
        let readyElapsed = Date().timeIntervalSince(readyBegan)
        ...
        await revealWeb(webView, readyElapsed: readyElapsed, began: began)
        session.recordLaunch()
    }
    ...
    private func revealWeb(
        ...
        let instant =
            tier == .warm
            && readyElapsed < Timeouts.warmInstantThreshold
