...
final class SessionStateTests: XCTestCase {
    ...
    func testSettledKeepsQuery() {
        let attributed = "https://example.test/path?sub1=abc&p=p1"
        var policy = SessionState()
        policy.rememberEntry(entry)
        policy.recordSuccess()
    ...
    func testUserDefaultsKeysUseBrandPrefix() {
        XCTAssertTrue(AppKeys.cacheEntryKey.hasPrefix("cs_"))
        XCTAssertTrue(AppKeys.committedKey.hasPrefix("cs_"))
        XCTAssertTrue(AppKeys.launchCountKey.hasPrefix("cs_"))
        XCTAssertFalse(AppKeys.cacheEntryKey.hasPrefix("exp_"))
    }
}
